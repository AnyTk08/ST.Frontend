...
const SubNavLv1Mobile: React.FC<SubNavProps> = ({
  dataMenu,
  sidebar,
  fnShowSidebar,
  LayoutMin900,
  ...
  const showSubnav = (pathURL: string | undefined, dataMenu) => {
    ...
    // if (dataMenu.nMenuID != 10 && dataMenu.nMenuID != 9)
    // {
    //     setIsDrawerMenu(false)
    // }
    // if ((dataMenu.nMenuID == 10 || dataMenu.nMenuID == 9) && subnav == false && LayoutMin900 == true) {
    //   setTimeout(() => {
  ...
  const IconRender = (name) => {
    ...
    return name != null ? <Icon style={{ color: '#1e456c' }} /> : <ManageAccountsRounded />
  }
              ...
              >
                ...
                <SidebarLabel sidebar={sidebar ? 1 : 0}>
                  <WrapMenuContent subnav={subnav ? 1 : 0}>
                    <Typography variant="subtitle1" style={{ color: '#1e456c' }}>
...
export default SubNavLv1Mobile;
