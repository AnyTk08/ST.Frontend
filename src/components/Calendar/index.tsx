...
import { CalendarProps } from "./CalendarClass";
//Icon
...
export default function Calendar(props: CalendarProps) {
    ...
    const renderEventContent = (eventInfo) => {
        ...
        let  spanTitle =  <span className="spanTitle">{title}</span>
        let  spantimeText = <span className="spanTitle">{timeText} : {title}</span>
        return <>
                                ...
                                <p className="calendar-title">
                                    {IsMeetingRoom ? (
                                        (nStatus == 0) ?
                                            (
                                                <>
                                                    <span className="spanIcon"><HourglassTopRoundedIcon htmlColor="#FCEE9E" className="HourglassTopRoundedIconClass" /></span>
                                                    {spanTitle}
                                                </>
                                            )
                                            :
                                            (
                                                <>
                                                    <span className="spanIcon"><CheckRoundedIcon htmlColor="#FFFFFF" className="CheckCircleRoundedIconClass" /></span>
                                                    {spanTitle}
                                                </>
                                            )
                                    ) : (
