...
export default function Template(props) {
  ...
  const Submit_WARNNING_Open = useSelector(
    AuthenSelectors.selectSubmit_WARNNING_Open
  );
  const Submit_WARNNING_Msg = useSelector(
    AuthenSelectors.selectSubmit_WARNNING_Msg
  );
  const Submit_WARNNING_Fn = useSelector(
    AuthenSelectors.selectSubmit_WARNNING_Fn
  );
  const Submit_WARNNING_FnCancel = useSelector(
    AuthenSelectors.selectSubmit_WARNNING_FnCancel
  );
  
  const Submit_WARNNING_Isload = useSelector(
    AuthenSelectors.selectSubmit_WARNNING_Isload
  );
  const Submit_SUCCESS_Fn = useSelector(
    AuthenSelectors.selectSubmit_SUCCESS_Fn
  );
  useEffect(() => { }, [SubmitIsload]);
  return (
    <Fragment>
      <DialogSuccess
      ...
      <DialogWarning
        ...
        sMsg={WarningMsg}
      />
      <DialogError handleClick={(c) => { }} open={ErrorOpen} sMsg={ErrorMsg} />
      <DialogSubmit
        open={SubmitOpen}
        sMsg={SubmitMsg}
